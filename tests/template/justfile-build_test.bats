...
@test "build: passes the image identity build args bootc relies on" {
    ...
    [[ "${args}" == *"--build-arg IMAGE_NAME=frameless"* ]]
...
@test "build: falls back to the GitHub repository owner for the vendor" {
    ...
    GITHUB_REPOSITORY_OWNER="acme-org" run_just build frameless stable
    [ "$status" -eq 0 ]

    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--build-arg IMAGE_VENDOR=acme-org"* ]]
    [[ "${args}" == *"--cache-from ghcr.io/acme-org/frameless"* ]]
}

@test "build: honours IMAGE_VENDOR and UBLUE_IMAGE_TAG overrides" {
    IMAGE_VENDOR="acme" UBLUE_IMAGE_TAG="pinned" run_just build frameless stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
...
@test "build: strips a local registry prefix from the image identity" {
    ...
    run_just build localhost/frameless stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--build-arg IMAGE_NAME=frameless"* ]]
    [[ "${args}" == *"--tag localhost/frameless:stable"* ]]
}

@test "build: forwards GITHUB_TOKEN as a build secret when set" {
    GITHUB_TOKEN="s3cret" run_just build frameless stable
    [ "$status" -eq 0 ]
...
@test "build: forwards explicit Containerfile metadata overrides" {
    IMAGE_DESC="Custom image" IMAGE_LOGO_URL="https://example.com/logo.svg" IMAGE_KEYWORDS="bootc,custom" IMAGE_REF="feature" run_just build frameless stable
    [ "$status" -eq 0 ]
    local args
    args="$(podman_build_args)"
...
@test "build: reads the layer cache but never writes it by default" {
    ...
    local args
    args="$(podman_build_args)"
    [[ "${args}" == *"--cache-from ghcr.io/projectbluefin/frameless"* ]]
    [[ "${args}" != *"--cache-to"* ]]
}

@test "build: writes the layer cache when REGISTRY_CACHE_WRITE=1" {
    REGISTRY_CACHE_WRITE=1 run_just build frameless stable
    [ "$status" -eq 0 ]
    [[ "$(podman_build_args)" == *"--cache-to ghcr.io/projectbluefin/frameless"* ]]
}
...
@test "tag-images: rejects an empty default tag" {
    run_just tag-images frameless "" "one two"
    [ "$status" -eq 1 ]
    [[ "$output" == *"Usage: just tag-images"* ]]
}

@test "tag-images: rejects an empty tag list" {
    run_just tag-images frameless stable ""
    [ "$status" -eq 1 ]
    [[ "$output" == *"Usage: just tag-images"* ]]
}

@test "tag-images: untags the default tag before re-tagging by image id" {
    run_just tag-images frameless stable "latest 44"
    [ "$status" -eq 0 ]

    mapfile -t calls <"${PODMAN_LOG}"
    [ "${calls[0]}" = "inspect localhost/frameless:stable" ]
    [ "${calls[1]}" = "untag localhost/frameless:stable" ]
    [ "${calls[2]}" = "tag sha256:deadbeef frameless:latest" ]
    [ "${calls[3]}" = "tag sha256:deadbeef frameless:44" ]
}

@test "tag-images: re-applies the default tag so local lookups still resolve" {
    run_just tag-images frameless stable "latest"
    [ "$status" -eq 0 ]
    [[ "$(tail -n1 "${PODMAN_LOG}")" = "tag sha256:deadbeef frameless:stable" ]]
    [[ "$output" == *"Tagged frameless with: latest"* ]]
}
