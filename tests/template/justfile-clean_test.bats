...
@test "clean: removes top-level entries whose name contains _build" {
	mkdir -p "${SANDBOX}/frameless_build/layer"
	touch "${SANDBOX}/frameless_build/layer/blob" \
...
@test "clean: succeeds on an already-clean tree and is repeatable" {
	mkdir -p "${SANDBOX}/frameless_build"
...
@test "sudo-clean: propagates the sudoif failure instead of cleaning unprivileged" {
	...
	mkdir -p "${SANDBOX}/frameless_build" "${SANDBOX}/output"
	touch "${SANDBOX}/frameless_build/blob" "${SANDBOX}/output/manifest"
