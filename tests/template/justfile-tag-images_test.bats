...
@test "tag-images: rejects a missing default tag" {
	run_tag_images frameless "" "t1 t2"
	[ "$status" -eq 1 ]
...
@test "tag-images: rejects an empty tag list" {
	run_tag_images frameless stable ""
	[ "$status" -eq 1 ]
	[[ "$output" == *"Usage: just tag-images"* ]]
	[ ! -s "${PODMAN_LOG}" ]
}
...
@test "tag-images: resolves the image id from the localhost-qualified default tag" {
	run_tag_images frameless stable "stable-42 latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'inspect localhost/frameless:stable'
}

@test "tag-images: untags the localhost default reference before re-tagging" {
	run_tag_images frameless stable "stable-42"
	[ "$status" -eq 0 ]
	...
	podman_calls | grep -Fxq 'untag localhost/frameless:stable'
}

@test "tag-images: applies every tag in the whitespace-separated list" {
	run_tag_images frameless stable "stable-42 stable-42.20250101 latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'tag sha256:deadbeef frameless:stable-42'
	podman_calls | grep -Fxq 'tag sha256:deadbeef frameless:stable-42.20250101'
	podman_calls | grep -Fxq 'tag sha256:deadbeef frameless:latest'
}

@test "tag-images: re-applies the default tag so local lookups keep working" {
	run_tag_images frameless stable "latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'tag sha256:deadbeef frameless:stable'

	# The default tag is restored last, after every alias tag.
	local last_tag
	last_tag="$(grep '^tag ' "${PODMAN_LOG}" | tail -n1)"
	[ "${last_tag}" = 'tag sha256:deadbeef frameless:stable' ]
}
...
@test "tag-images: collapses repeated whitespace in the tag list" {
	run_tag_images frameless stable "  latest   stable-42  "
	[ "$status" -eq 0 ]
...
@test "tag-images: reports the tags it applied" {
	run_tag_images frameless stable "latest stable-42"
	[ "$status" -eq 0 ]
	[[ "$output" == *"Tagged frameless with: latest stable-42"* ]]
}
...
STUB
	...
	PODMAN=docker-shim run_tag_images frameless stable "latest"
	[ "$status" -eq 0 ]
	podman_calls | grep -Fxq 'shim inspect localhost/frameless:stable'
	podman_calls | grep -Fxq 'shim tag sha256:cafe frameless:latest'
