...
@test "00-image-info: writes Bluefin-compatible image metadata" {
    ...
    [ "$status" -eq 0 ]
    [ "$(json_field image-name)" = "frameless" ]
    [ "$(json_field image-vendor)" = "projectbluefin" ]
    [ "$(json_field image-ref)" = "ostree-image-signed:docker://ghcr.io/projectbluefin/frameless" ]
...
@test "00-image-info: does not create a flavor field from the image name" {
    export IMAGE_NAME="frameless-nvidia"
    run_script
    [ "$status" -eq 0 ]
    [ "$(json_field image-ref)" = "ostree-image-signed:docker://ghcr.io/projectbluefin/frameless-nvidia" ]
...
@test "00-image-info: updates existing base os-release identity" {
    ...
    grep -q '^VARIANT_ID="frameless"$' "${OS_RELEASE}"
    grep -q '^PRETTY_NAME="frameless (Version: 44.20260907.1)"$' "${OS_RELEASE}"
    grep -q '^NAME="frameless"$' "${OS_RELEASE}"
    grep -q '^VERSION="44.20260907.1 (silverblue)"$' "${OS_RELEASE}"
    grep -q '^OSTREE_VERSION="44.20260907.1"$' "${OS_RELEASE}"
    grep -q '^IMAGE_ID="frameless"$' "${OS_RELEASE}"
...
@test "00-image-info: derives GitHub URLs from image identity" {
    ...
    grep -q '^HOME_URL="https://github.com/projectbluefin/frameless"$' "${OS_RELEASE}"
    grep -q '^DOCUMENTATION_URL="https://github.com/projectbluefin/frameless/blob/main/README.md"$' "${OS_RELEASE}"
    grep -q '^SUPPORT_URL="https://github.com/projectbluefin/frameless/issues"$' "${OS_RELEASE}"
    grep -q '^BUG_REPORT_URL="https://github.com/projectbluefin/frameless/issues/new"$' "${OS_RELEASE}"
    grep -q '^ID_LIKE="fedora"$' "${OS_RELEASE}"
}

@test "00-image-info: permits explicit URL overrides without changing base identity" {
    export HOME_URL="https://frameless.example"
    export DOCUMENTATION_URL="https://docs.frameless.example"
    export SUPPORT_URL="https://support.frameless.example"
    export BUG_REPORT_URL="https://bugs.frameless.example"
    run_script
    [ "$status" -eq 0 ]

    grep -q '^PRETTY_NAME="frameless (Version: stable)"$' "${OS_RELEASE}"
    grep -q '^NAME="frameless"$' "${OS_RELEASE}"
    grep -q '^ID_LIKE="fedora"$' "${OS_RELEASE}"
    grep -q '^HOME_URL="https://frameless.example"$' "${OS_RELEASE}"
    grep -q '^DOCUMENTATION_URL="https://docs.frameless.example"$' "${OS_RELEASE}"
    grep -q '^SUPPORT_URL="https://support.frameless.example"$' "${OS_RELEASE}"
    grep -q '^BUG_REPORT_URL="https://bugs.frameless.example"$' "${OS_RELEASE}"
}
...
@test "00-image-info: escapes JSON values" {
    export IMAGE_PRETTY_NAME='Frameless "OS"'
    export IMAGE_NAME='frameless"test'
    run_script
    [ "$status" -eq 0 ]

    run python3 -m json.tool "${IMAGE_INFO_JSON}"
    [ "$status" -eq 0 ]
    [ "$(json_field image-name)" = 'frameless"test' ]
}
...
@test "00-image-info: replaces an existing key whose value contains sed metacharacters" {
    export HOME_URL="https://frameless.example/?a=1&b=2|c"
    run_script
    [ "$status" -eq 0 ]

    grep -qF 'HOME_URL="https://frameless.example/?a=1&b=2|c"' "${OS_RELEASE}"
}
